...
void bfs(int sr, int sc, int left, int right)
{
    ...
    vis[sr][sc] = true;
    int up = sr, down = sr;
    while (up >= 0 && up <= row && arr[up][sc] != '*')
    {
        vis[up][sc] = true;
        if (l_left[up][sc] >= left)
        {
        }
        else
        {
    ...
    while (down >= 0 && down <= row && arr[down][sc] != '*')
    {
        vis[down][sc] = true;
        if (l_left[down][sc] >= left)
        {
        }
        else
        {
            l_left[down][sc] = left;
            bfs(down, sc - 1, left - 1, right);
        }
        down--;
    }
    ...
    {
        vis[down][sc] = true;
        if (r_right[down][sc] >= right)
        {
        }
        else
        {
            r_right[down][sc] = left;
            bfs(down, sc + 1, left, right - 1);
        }
        down--;
    }
}
